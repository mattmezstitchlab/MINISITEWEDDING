...
export default function Onboarding() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1A1A1A] flex flex-col overflow-x-hidden" style={{ fontFamily: studioFont }}>
      <nav className="min-h-[64px] sm:min-h-[68px] flex items-center justify-between gap-2 px-3 sm:px-8 py-2.5 border-b border-black/5 bg-[#FAF8F5]/85 backdrop-blur-xl sticky top-0 z-40">
        <Link to="/" className="flex items-center gap-2 shrink-0">
          <span className="w-8 h-8 rounded-full bg-neutral-900 text-white flex items-center justify-center text-[13px] font-semibold" style={{ fontFamily: studioFont }}>W</span>
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div key="s0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.35 }}>
                <h1 className="text-center font-medium tracking-tight" style={{ fontFamily: studioFont, fontSize: 'clamp(1.9rem, 6vw, 3.2rem)' }}>Qui se marie ?</h1>
                ...
                <div className="mt-8 sm:mt-10 grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={partner1} onChange={(e) => setPartner1(e.target.value)} placeholder="Marie" autoFocus className={inputCls} style={{ fontFamily: studioFont, fontSize: '1.4rem' }} />
                  </div>
                  <div>
                    <label className="block text-[11px] tracking-[0.25em] uppercase text-neutral-400 font-semibold mb-2 ml-1">Second prénom</label>
                    <input value={partner2} onChange={(e) => setPartner2(e.target.value)} placeholder="Matt" className={inputCls} style={{ fontFamily: studioFont, fontSize: '1.4rem' }} />
                  </div>
                </div>
                {(partner1 || partner2) && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 text-center p-6 rounded-3xl bg-white border border-black/5 shadow-sm">
                    ...
                    <div className="mt-2 font-medium tracking-tight" style={{ fontFamily: studioFont, fontSize: 'clamp(2rem, 6vw, 3.6rem)' }}>{partner1 || '…'} <span className="font-light text-[#8A6D4B]">&</span> {partner2 || '…'}</div>
                  </motion.div>
                )}
              </motion.div>
            )}
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.35 }} className="text-center">
                <h1 className="font-medium tracking-tight" style={{ fontFamily: studioFont, fontSize: 'clamp(1.9rem, 6vw, 3.2rem)' }}>Quelle est la date ?</h1>
                ...
                <div className="mt-8 sm:mt-10 max-w-md mx-auto">
                  <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-5 sm:px-6 py-4 sm:py-5 rounded-2xl bg-white border border-black/10 text-lg sm:text-xl outline-none focus:border-black/40 transition text-center font-medium" style={{ fontFamily: studioFont }} />
                  {date && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 p-5 sm:p-6 rounded-3xl bg-white border border-black/10 shadow-sm">
                      <div className="capitalize text-lg sm:text-xl font-medium tracking-tight" style={{ fontFamily: studioFont }}>{formatDateLong(date)}</div>
            ...
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.35 }}>
                <h1 className="text-center font-medium tracking-tight" style={{ fontFamily: studioFont, fontSize: 'clamp(1.9rem, 6vw, 3.2rem)' }}>Où cela se passe ?</h1>
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.35 }}>
                <h1 className="text-center font-medium tracking-tight" style={{ fontFamily: studioFont, fontSize: 'clamp(1.9rem, 6vw, 3.2rem)' }}>Quel style vous ressemble ?</h1>
                  ...
                  {WEDDING_STYLES.map((s) => (
                    <button key={s.id} onClick={() => setStyle(s.id)} className={`relative aspect-[3/4] rounded-2xl sm:rounded-3xl overflow-hidden text-left transition ring-offset-2 ring-offset-[#FAF8F5] ${style === s.id ? 'ring-[3px] ring-neutral-900 scale-[1.01]' : 'hover:scale-[1.01]'}`}>
                      ...
                      <span className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 text-white">
                        <span className="block text-base sm:text-lg font-medium tracking-tight" style={{ fontFamily: studioFont }}>{s.name}</span>
