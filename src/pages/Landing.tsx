...
export default function Landing() {
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1A1A1A] overflow-x-hidden" style={{ fontFamily: frauncesFont }}>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#FAF8F5]/85 backdrop-blur-xl border-b border-black/5">
        <div className="max-w-6xl mx-auto px-4 sm:px-8 h-[64px] sm:h-[68px] flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-full bg-neutral-900 text-white flex items-center justify-center text-[13px] font-normal" style={{ fontFamily: frauncesFont }}>W</span>
      ...
      <header className="relative min-h-[100svh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative text-center px-4 sm:px-6 pt-24 pb-16 sm:py-0 max-w-4xl mx-auto w-full">
          <motion.h1
            ...
            style={{ fontFamily: frauncesFont, fontSize: 'clamp(2.5rem, 8vw, 5.5rem)' }}
          >
      ...
      <section className="py-16 sm:py-28 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp} transition={{ duration: 0.7 }} className="text-center max-w-2xl mx-auto">
            <div className="text-[11px] tracking-[0.3em] uppercase text-[#8A6D4B] font-medium">Compris en 10 secondes</div>
            <h2 className="mt-3 font-light" style={{ fontFamily: frauncesFont, fontSize: 'clamp(1.9rem, 5vw, 3.2rem)' }}>Quatre gestes. Zéro effort.</h2>
          </motion.div>
          <div className="mt-10 sm:mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {STEPS.map((s, i) => (
              <motion.div key={s.n} {...fadeUp} transition={{ duration: 0.6, delay: i * 0.1 }} className="p-6 sm:p-8 rounded-3xl bg-white border border-black/5 shadow-[0_10px_40px_rgba(0,0,0,0.04)]">
                <div className="text-[13px] tracking-[0.25em] text-[#8A6D4B] font-medium" style={{ fontFamily: frauncesFont }}>{s.n}</div>
                <div className="mt-2 text-2xl font-light" style={{ fontFamily: frauncesFont }}>{s.title}</div>
                <p className="mt-2 text-sm text-neutral-500 leading-relaxed font-light">{s.text}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="styles" className="py-16 sm:py-28 bg-white border-y border-black/5 overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <motion.div {...fadeUp} transition={{ duration: 0.7 }} className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <div className="text-[11px] tracking-[0.3em] uppercase text-[#8A6D4B] font-medium">Huit directions artistiques</div>
              <h2 className="mt-3 font-light" style={{ fontFamily: frauncesFont, fontSize: 'clamp(1.9rem, 5vw, 3.2rem)' }}>Choisissez votre style.<br />Le reste est automatique.</h2>
            </div>
            ...
            <motion.div key={s.id} {...fadeUp} transition={{ duration: 0.6, delay: (i % 4) * 0.08 }} className="shrink-0 w-[230px] sm:w-[280px] group">
              <div className="relative aspect-[3/4] rounded-3xl overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 text-white">
                  <div className="text-xl sm:text-2xl font-light" style={{ fontFamily: frauncesFont }}>{s.name}</div>
                  <div className="mt-1 text-[12px] sm:text-[13px] text-white/80 font-light">{s.tagline}</div>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <section id="experience" className="py-16 sm:py-28 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <motion.div {...fadeUp} transition={{ duration: 0.7 }}>
            <div className="text-[11px] tracking-[0.3em] uppercase text-[#8A6D4B] font-medium">L’éditeur</div>
            <h2 className="mt-3 font-light" style={{ fontFamily: frauncesFont, fontSize: 'clamp(1.9rem, 5vw, 3.2rem)' }}>Choisir. Modifier.<br />Voir. Publier.</h2>
            <p className="mt-4 text-neutral-500 leading-relaxed max-w-md text-sm sm:text-base font-light">Pas de panneau rempli de paramètres. Au centre, votre site en direct. Un clic sur une photo ouvre vos images, un clic sur un texte ouvre l’éditeur. Conçu pour mobile et ordinateur.</p>
            <div className="mt-6 sm:mt-8 space-y-3 font-light">
              {['Sections réorganisables par glisser-déposer', 'Réglages contextuels instantanés', 'Aperçu mobile et ordinateur en temps réel'].map((t) => (
                <div key={t} className="flex items-center gap-3 text-sm sm:text-[15px]"><span className="w-6 h-6 rounded-full bg-[#8A6D4B]/10 text-[#8A6D4B] flex items-center justify-center text-[12px] font-medium">✓</span>{t}</div>
              ))}
            </div>
          ...
          <motion.div {...fadeUp} transition={{ duration: 0.8 }} className="relative">
            <div className="rounded-[2rem] overflow-hidden shadow-[0_30px_80px_rgba(0,0,0,0.12)] border border-black/5 relative">
              ...
              <div className="absolute bottom-5 sm:bottom-6 left-5 sm:left-6 right-5 sm:right-6 flex items-center justify-between text-white">
                <div><div className="text-[11px] tracking-[0.25em] uppercase text-white/70">Matt & Marie</div><div className="text-xl sm:text-2xl font-light" style={{ fontFamily: frauncesFont }}>18.07.2027</div></div>
                <Link to="/p/matt-marie" className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-white text-neutral-900 text-[13px] font-medium">Voir</Link>
              </div>
            </div>
      ...
      <section id="modules" className="py-16 sm:py-28 px-4 sm:px-6 bg-[#141311] text-white">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp} transition={{ duration: 0.7 }} className="text-center max-w-2xl mx-auto">
            <div className="text-[11px] tracking-[0.3em] uppercase text-[#C6A15B] font-medium">Tout est inclus</div>
            <h2 className="mt-3 font-light" style={{ fontFamily: frauncesFont, fontSize: 'clamp(1.9rem, 5vw, 3.2rem)' }}>Un objet éditorial complet</h2>
            <p className="mt-3 text-white/60 leading-relaxed text-sm sm:text-base font-light">RSVP, cagnotte, galerie, programme, FAQ — chaque module est opérationnel immédiatement. Vous personnalisez à votre rythme.</p>
          </motion.div>
          <div className="mt-10 sm:mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {MODULES.map((m, i) => (
              <motion.div key={m.title} {...fadeUp} transition={{ duration: 0.6, delay: (i % 3) * 0.1 }} className="p-6 sm:p-8 rounded-3xl bg-white/5 border border-white/10 hover:bg-white/10 transition">
                ...
                <div className="mt-4 text-xl font-light" style={{ fontFamily: frauncesFont }}>{m.title}</div>
                <p className="mt-2 text-[14px] text-white/55 leading-relaxed font-light">{m.text}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-28 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div {...fadeUp} transition={{ duration: 0.7 }} className="text-center">
            <div className="text-[11px] tracking-[0.3em] uppercase text-[#8A6D4B] font-medium">Les trois temps</div>
            <h2 className="mt-3 font-light" style={{ fontFamily: frauncesFont, fontSize: 'clamp(1.9rem, 5vw, 3.2rem)' }}>Un site qui vit avec vous</h2>
          </motion.div>
          <div className="mt-10 sm:mt-12 grid md:grid-cols-3 gap-4">
            {PHASES.map((p, i) => (
              <motion.div key={p.id} {...fadeUp} transition={{ duration: 0.6, delay: i * 0.1 }} className="p-6 sm:p-8 rounded-3xl bg-white border border-black/5 shadow-sm text-center">
                <div className="text-[11px] tracking-[0.3em] uppercase text-neutral-400 font-medium">Phase {i + 1}</div>
                <div className="mt-2 text-2xl sm:text-3xl font-light" style={{ fontFamily: frauncesFont }}>{p.name}</div>
                <p className="mt-2 text-sm text-neutral-500 leading-relaxed font-light">{p.desc}</p>
              </motion.div>
            ))}
          </div>
          <motion.p {...fadeUp} transition={{ duration: 0.7 }} className="mt-8 text-center text-neutral-500 text-sm sm:text-base italic max-w-lg mx-auto" style={{ fontFamily: frauncesFont }}>« Après le mariage, il devient la mémoire numérique de votre grand jour. »</motion.p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div {...fadeUp} transition={{ duration: 0.8 }} className="relative rounded-3xl overflow-hidden aspect-[16/9] sm:aspect-[21/9]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-white text-center">
              <div className="text-[11px] tracking-[0.3em] uppercase text-[#C6A15B] font-medium">Voir le rendu invité</div>
              <div className="mt-2 font-light text-2xl sm:text-4xl" style={{ fontFamily: frauncesFont }}>Marie & Matt<br /><span className="text-base sm:text-xl font-light opacity-80">18 Juillet 2027</span></div>
              <Link to="/p/matt-marie" className="mt-5 inline-flex items-center gap-2 px-7 sm:px-8 py-3 rounded-full bg-white text-neutral-900 text-sm font-medium hover:bg-neutral-100 transition shadow-lg">Voir l’exemple <ArrowRight size={15} /></Link>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 bg-[#FAF8F5] text-center border-t border-black/5">
        <div className="max-w-2xl mx-auto">
          <div className="w-12 h-12 rounded-full bg-neutral-900 text-white flex items-center justify-center mx-auto text-lg font-light" style={{ fontFamily: frauncesFont }}>W</div>
          <h2 className="mt-5 font-light" style={{ fontFamily: frauncesFont, fontSize: 'clamp(1.9rem, 5vw, 3.2rem)' }}>Le site de votre mariage,<br />en quelques instants.</h2>
          <Link to="/creer" className="mt-7 inline-flex items-center gap-2 px-8 sm:px-10 py-4 rounded-full bg-neutral-900 text-white text-sm tracking-[0.12em] uppercase font-medium hover:bg-neutral-700 transition shadow-xl">Créer mon site <ArrowRight size={16} /></Link>
        </div>
      </section>
      ...
      <footer className="py-8 px-4 sm:px-8 border-t border-black/5 text-sm text-neutral-400 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
        <div className="flex items-center gap-2"><span className="w-6 h-6 rounded-full bg-neutral-900 text-white flex items-center justify-center text-[11px]" style={{ fontFamily: frauncesFont }}>W</span><span className="tracking-[0.25em] uppercase font-medium">Wedding Site</span></div>
