...
import {
  ARTICLES as ARTICLES_DU_MAGASIN,
  CONVIVES,
  MAGASIN,
  PACKAGES as MENUS_DU_MAGASIN,
  RAYONS as RAYONS_DU_MAGASIN,
  articleParId,
  euros,
  prixDeLArticle,
} from './superMariage';
...
/** **L'encre d'un fond** : noire sur un fond clair, blanche sur un fond sombre. */
function encreSur(fond: string): string {
  const hex = fond.replace('#', '');
  if (hex.length < 6) return ENCRE_CLAIRE;
  const r = parseInt(hex.slice(0, 2), 16);
  const v = parseInt(hex.slice(2, 4), 16);
  const b = parseInt(hex.slice(4, 6), 16);
  return (0.299 * r + 0.587 * v + 0.114 * b) / 255 > 0.62 ? ENCRE_SOMBRE : ENCRE_CLAIRE;
}
...
export function mondeDOuverture(date: Date = new Date()): Monde {
  ...
  return {
    ...
    cases: [
      ...
      caseSimple('porte-magasin', MAGASIN.nom, 'prix', teinte(25).fond, {
        surTitre: 'LA CAISSE',
        sousTitre: `le mariage entier, au prix où il se fait — ${MAGASIN.caisse}`,
        encre: encreSur(teinte(25).fond),
        ouvre: 'magasin',
        detail: [
          { label: 'ARTICLES', valeur: String(ARTICLES_DU_MAGASIN.length) },
          { label: 'RAYONS', valeur: String(RAYONS_DU_MAGASIN.length) },
        ],
      }),
...
/* ——————————————————— LE MAGASIN : LE MARIAGE, À LA CAISSE ——————————————————— */

/** La famille d'une ligne de caisse : ce que le magasin montre, et à qui. */
export function familleDuneLigne(id: string): Famille {
  if (id.startsWith('sup-')) return 'public';
  if (id.startsWith('horaire-')) return 'invites';
  if (id.startsWith('menu-') || id.startsWith('menu')) return 'famille';
  return 'prive';
}

/** Ce qu'une ligne est, dans le magasin : une heure, un métier, un petit prix. */
function moduleDuneLigne(id: string): ModuleDeCase {
  if (id.startsWith('horaire-')) return 'date';
  if (id.startsWith('sup-')) return 'produit';
  if (id.startsWith('menu')) return 'prix';
  return 'personne';
}

/**
 * **LE MAGASIN** — le mariage entier, au prix où il se fait.
 *
 * Chaque rayon est une case — il s'ouvre sur son propre monde ; chaque article
 * du rayon est une case aussi — on la prend, et la caisse calcule. Le ticket
 * n'est pas une image : c'est la somme exacte de ce qu'on a coché.
 */
export function mondeDuMagasin(): Monde {
  const cases: CaseDuMonde[] = [
    caseSimple('caisse', 'LE TICKET', 'carte', teinte(22).fond, {
      surTitre: 'LA CAISSE',
      sousTitre: `${ARTICLES_DU_MAGASIN.length} articles · ${MENUS_DU_MAGASIN.length} menus`,
      encre: encreSur(teinte(22).fond),
      detail: [
        { label: 'CONVIVES', valeur: `${CONVIVES} personnes` },
        { label: 'CAISSE', valeur: MAGASIN.caisse },
      ],
    }),
  ];

  RAYONS_DU_MAGASIN.forEach((rayon, i) => {
    const palette = teinte(3 + i * 4);
    cases.push(
      caseSimple(`rayon-${rayon.key}`, rayon.label.toUpperCase(), 'bouton', palette.fond, {
        surTitre: 'LE RAYON',
        sousTitre: rayon.sousTitre,
        encre: encreSur(palette.fond),
        ouvre: `rayon-${rayon.key}`,
        detail: [
          { label: 'ARTICLES', valeur: String(rayon.articles.length) },
          { label: 'CHEZ', valeur: MAGASIN.nom },
        ],
      }),
    );
    rayon.articles.forEach((article) => {
      const teinteDeLArticle = teinte(6 + i * 3);
      cases.push(
        caseSimple(`ligne-${article.id}`, article.label.toUpperCase(), moduleDuneLigne(article.id), teinteDeLArticle.fond, {
          surTitre: rayon.label.toUpperCase(),
          sousTitre: `${euros(prixDeLArticle(article))}${article.quantite ? ` · ×${article.quantite}` : ''}`,
          encre: encreSur(teinteDeLArticle.fond),
          famille: familleDuneLigne(article.id),
          detail: [
            { label: 'PRIX', valeur: euros(article.prix) },
            { label: 'UNITÉ', valeur: article.quantite ? `${article.quantite} convives` : 'à la pièce' },
            ...(article.univers ? [{ label: 'UNIVERS', valeur: article.univers }] : []),
            ...(article.promo ? [{ label: 'PROMO', valeur: 'rayon 7' }] : []),
          ],
        }),
      );
    });
  ...
  return {
    id: 'magasin',
    titre: MAGASIN.nom,
    sous: `${RAYONS_DU_MAGASIN.length} rayons · ${MAGASIN.caisse}`,
    quoi: 'on coche, et le ticket s’imprime',
    cases,
  };
}

/** **Un rayon** : ses articles, tous, et la caisse au bout. */
export function mondeDUnRayon(key: string): Monde {
  const rayon = RAYONS_DU_MAGASIN.find((r) => r.key === key);
  if (!rayon) return mondeDuMagasin();
  const cases: CaseDuMonde[] = [
    caseSimple('caisse', 'LE TICKET', 'carte', teinte(22).fond, {
      surTitre: 'LA CAISSE',
      sousTitre: rayon.label.toUpperCase(),
      encre: encreSur(teinte(22).fond),
    }),
    ...rayon.articles.map((article, i) =>
      caseSimple(`ligne-${article.id}`, article.label.toUpperCase(), moduleDuneLigne(article.id), teinte(6 + i).fond, {
        surTitre: rayon.label.toUpperCase(),
        sousTitre: euros(prixDeLArticle(article)),
        encre: encreSur(teinte(6 + i).fond),
        famille: familleDuneLigne(article.id),
        detail: [{ label: 'PRIX', valeur: euros(article.prix) }],
      }),
    ),
  ];
  return { id: `rayon-${rayon.key}`, titre: rayon.label.toUpperCase(), sous: rayon.sousTitre, quoi: 'un article par case', cases };
}

/** **Une ligne** : ce qu'elle coûte, d'où elle vient, et ce qui va avec. */
export function mondeDUneLigne(id: string): Monde {
  const article = articleParId(id);
  if (!article) return mondeDuMagasin();
  const rayon = RAYONS_DU_MAGASIN.find((r) => r.articles.some((a) => a.id === id));
  const cases: CaseDuMonde[] = [
    caseSimple('ligne', article.label.toUpperCase(), moduleDuneLigne(article.id), teinte(9).fond, {
      surTitre: rayon?.label.toUpperCase() ?? MAGASIN.nom,
      sousTitre: euros(prixDeLArticle(article)),
      encre: encreSur(teinte(9).fond),
      famille: familleDuneLigne(article.id),
      detail: [
        { label: 'PRIX', valeur: euros(article.prix) },
        { label: 'UNITÉ', valeur: article.quantite ? `${article.quantite} convives` : 'à la pièce' },
    ...
    caseSimple('prix', euros(prixDeLArticle(article)), 'prix', teinte(3).fond, {
      surTitre: 'LE PRIX',
      sousTitre: article.quantite ? `× ${article.quantite} convives` : 'à la pièce',
      encre: encreSur(teinte(3).fond),
    }),
    caseSimple('caisse', 'LE TICKET', 'carte', teinte(22).fond, {
      surTitre: 'LA CAISSE',
      sousTitre: MAGASIN.caisse,
      encre: encreSur(teinte(22).fond),
    }),
    ...(rayon?.articles ?? [])
      .filter((a) => a.id !== id)
      .slice(0, 6)
      .map((a, i) =>
        caseSimple(`ligne-${a.id}`, a.label.toUpperCase(), moduleDuneLigne(a.id), teinte(6 + i * 2).fond, {
          surTitre: rayon?.label.toUpperCase(),
          sousTitre: euros(prixDeLArticle(a)),
          encre: encreSur(teinte(6 + i * 2).fond),
          famille: familleDuneLigne(a.id),
        }),
      ),
  ];
  return { id: `ligne-${id}`, titre: article.label.toUpperCase(), sous: article.detail, quoi: 'l’article, son prix, et sa caisse', cases };
}
...
export function mondeDeLId(id: string | null | undefined, date: Date = new Date()): Monde {
  ...
  if (id === 'magasin') return mondeDuMagasin();
  ...
  const rayon = /^rayon-(.+)$/.exec(id);
  if (rayon) return mondeDUnRayon(rayon[1]!);

  const ligne = /^ligne-(.+)$/.exec(id);
  if (ligne) return mondeDUneLigne(ligne[1]!);
...
export const NŒUDS_DU_MONDE: Array<{ id: string; titre: string; source: string }> = [
  ...
  { id: 'magasin', titre: 'LE MAGASIN', source: 'superMariage · la caisse' },
