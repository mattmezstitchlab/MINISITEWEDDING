...
> **Historique.** Ce chapitre décrit la page du 20 septembre 2026. Depuis le
> 21 septembre, **la machine n'est plus en haut de la page** : l'application
> *est* le ticket (voir §7). La machine reste dans le dépôt, entière, et son
> contrat reste vérifié — elle n'est simplement plus montée.
...
### Les bandes — la composition de la page

Autour de ces deux étages, la page est écrite **par bandes**, d'après une
référence étudiée le 20 septembre 2026 (Tapdaa) : une barre qui porte la marque,
un titre unique et sa phrase, une suite numérotée, trois colonnes égales, une
bande d'image calme, trois formules de même poids, des questions, un pied. Le
détail de ce qui a été pris — et de ce qui n'a **pas** été copié — est dans
**`docs/composition-tapdaa.md`** ; le contenu des bandes est dans
`src/lib/bandesDeLAime.ts`, et leur mise en page dans
`src/components/LesBandesDeLAime.tsx`.

### L'adresse

`?coches=…` reste le reçu. Elle porte aussi **la demande** (`?demande=diner`) :
un lien peut donc arriver avec le ticket déjà rempli, ou avec une demande à
faire passer.

## 7. L'APPLI EST LE TICKET — plein écran, long, vivant (21 septembre 2026)

> « Sans machine : l'appli, c'est le ticket plein écran. On scrolle un long
> ticket, même avec la musique, les heures, la totale — et on clique sur le
> picto tampon pour faire tampon “payé”. »

Le premier écran n'est plus une machine : c'est **le papier lui-même**, et il
prend l'écran. La machine de Ripple n'est plus montée sur la page — **rien n'est
détruit** : le composant est entier (`src/components/MachineDeRipple.tsx`), et
son contrat est vérifié directement dans les tests.

```
┌──────────────────────────────────────────┐
│            SUPER MARIAGE                 │  l'impression, en haut du papier
│      LE SPÉCIALISTE DU TICKET DE CAISSE  │
│      NUB-139 · CAISSE 3 · 23:00          │
│  ┌──── le visuel du jour, en coupon ────┐│
│  └──────────────────────────────────────┘│
│  LIGNES 5   LE MARIAGE 2 241 €           │
│  MIS DE CÔTÉ 900 €  RESTE À FINANCER …   │  ce qui reste est surligné au fluo
│  LA JOURNÉE          5/5                 │
│    22:00 ─●──── Ouverture — néons   350 €│  la timeline descend sur le papier
│    22:17 ─●──── Cérémonie — rayon 7 900 €│  « maintenant » est marqué
│  LA MUSIQUE          2/5                 │
│    1. CÉRÉMONIE              16:00       │  le plan de la nuit, morceau par morceau
│      Elvis Presley — Can't Help…     76  │
│  LES GENS · LA TABLE · LES PETITS PRIX · │  les 99 lignes, chacune une fois
│  LES DOCUMENTS · LE SITE · LE VOYAGE     │
│  TOTAL                          2 241 €  │
│  ▮▮▯▮▯▯▮▮▯▯  NUB-139                     │  le code-barres est fait du code
│  (✉)(★)(✈)(◉)(▤)(⧉)(◎)  partager  le site│  le pupitre, collé en bas
└──────────────────────────────────────────┘
```
...
| --- | --- |
| **on coche une ligne** | elle passe au **fluo** (`vp-fluo`), son ✓ s'imprime, et le total, la part du rêve et la barre se refont |
| **on tamponne** | les sept pictos du pupitre posent leur marque **sur le papier**, de travers, à l'encre : `PAYÉ`, `MERCI`, `DÉPART`, `REÇU`, `MA PLACE`, `AFFRANCHI`, `TOP` — et le tampon fait passer le ticket à **✓ PAYÉ · MERCI** |
| **on partage** | le lien du mini-site part complet (`?code=…&site=1&coches=…`) |

Ce qui est **imprimé** sur le papier, dans l'ordre : l'impression `SUPER
MARIAGE`, le code et la caisse, le coupon du jour, les quatre chiffres, **la
journée** (la timeline des heures), **la musique** (les neuf moments de la nuit
et leurs morceaux), **la table**, **les gens**, **les petits prix**, **les
documents**, **le site**, **le voyage**, les cinq papiers, le total, le
code-barres, et le pupitre.

**Détails d'objet** : papier ivoire à chasse fixe, dents en haut et en bas
(SVG), taille du texte en `clamp()` — tout le ticket grandit avec l'écran, et
sur un ordinateur il prend la hauteur de la fenêtre. `@media print` : seul le
papier s'imprime (`.vp-non-imprimable` disparaît), donc le ticket est **un vrai
objet à imprimer**.

| Le morceau | Le fichier |
| --- | --- |
| le ticket plein écran, ses sections, son pupitre, ses tampons | `src/components/LeTicketPleinEcran.tsx` |
| les mots tamponnés (`PAYÉ`, `MERCI`, `DÉPART`…) | `src/lib/marquesDuTicket.ts` |
