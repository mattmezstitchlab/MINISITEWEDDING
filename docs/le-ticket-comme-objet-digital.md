...
## 5. La landing — la capsule, le téléphone, et l'éditeur
...
La référence est un long blanc : un titre en sérif, une phrase, **un champ dans
une capsule et un bouton vert**, puis des blocs — une question, un libellé
minuscule, une capsule, `SUIVANT` — et un pied en tout petit. On suit ce dessin à
la lettre, et on y met le produit. **Deux adresses, un seul objet :**

| L'adresse | Ce qu'on y voit |
| --- | --- |
| **`/`** | la landing : la capsule, l'iPhone, le papier au propre, l'éditeur |
| **`/ticket`** (et `/caisse`) | **le papier seul**, avec son champ |

### 5.1 La capsule

`LaCapsule` : un libellé minuscule, **une forme arrondie** où l'on écrit, un
bouton vert — et rien autour. C'est le même objet partout (le code, les prénoms,
la ville, les invités, le rêve, les lignes) : `.vp-capsule`, `.vp-bouton-vert`,
`.vp-bouton-clair`, `.vp-petit-mot`.

### 5.2 L'iPhone, et le papier qui défile

`LIphone` n'est pas une image : **c'est le vrai ticket**, posé à l'échelle d'une
poche (`scale(0.62)` sur une feuille large de `296/0,62`), et il **défile tout
seul** (`@keyframes vp-defile`, 38 s, en aller-retour). On n'y touche pas
(`pointer-events: none`) : c'est une démonstration. Elle s'arrête net si l'on
demande à ne pas voir d'animation (`prefers-reduced-motion`).

### 5.3 Le ticket au propre, bien droit

Le papier est monté **deux fois** sur la landing : une dans le téléphone, une au
propre (`data-landing-papier`). Au propre il **ne penche plus** — la seule chose
de travers reste le tampon, et c'est lui qui doit l'être — et il ne prend plus
toute la hauteur de l'écran : `.vp-landing-papier .vp-comptoir { min-height: 0 }`.
Il se coche, il se paye, il se tourne : c'est l'objet, pas une capture.

### 5.4 L'éditeur — huit questions

`LEditeurDuTicket` : **huit questions**, chacune dans un bloc de la référence
(sérif, libellé, capsule, `SUIVANT`) — et l'éditeur ne fabrique pas un
formulaire : **il configure le ticket**. Chaque réponse écrit dans l'état
(`useLEtatDuTicket`), donc elle se pose dans le téléphone, sur le papier, et dans
l'adresse.

| La question | Ce qu'elle règle |
| --- | --- |
| C'est quoi, votre mariage ? | le code, la signature du papier |
| Qui se marie ? | les prénoms, imprimés sur le ticket et dans le code-barres |
| Où ça, alors ? | le lieu |
| Combien d'invités ? | le nombre, écrit à la main — il n'y a pas de liste |
| Vous en rêvez ? | le voyage, dans les mots des mariés |
| Et qu'est-ce qu'on prend ? | les lignes du magasin, et les opérations — **par l'agent** |
| La couleur du marker ? | le fluo qui surligne tout le papier |
| Qui regarde le papier ? | la face : le ticket, ou sa facture et son reçu |

`LES_QUESTIONS_DU_TICKET`, `data-question`, `data-question-suivant`,
`data-marker-editeur`, `data-face-choix`.
