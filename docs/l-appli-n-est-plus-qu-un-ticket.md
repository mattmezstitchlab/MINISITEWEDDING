...
## 5. La landing — vingt-et-un septembre, plus tard

> « Un vrai champ capsule, et sur une landing ? Comme la capture, et présentation
> dans un iPhone en animé… et du coup un ticket au propre bien droit, avec éditeur
> propre même design que sur l'exemple. »

`/` est maintenant **la landing** : la capsule (le code), l'**iPhone animé** qui
présente le vrai papier, le **ticket au propre** (bien droit, et il se manipule),
puis l'**éditeur** en huit questions, dans le dessin de la référence — blanc,
sérif, centré, un seul vert. `/ticket` garde **le papier seul**. Les deux montent
le même état (`useLEtatDuTicket`) et le même ticket : un objet, deux façons de le
montrer. Détail complet : `docs/le-ticket-comme-objet-digital.md` §5.
